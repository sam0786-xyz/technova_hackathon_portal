...
ADD COLUMN IF NOT EXISTS signature_region JSON;

-- 4. Create certificate_analytics table for detailed tracking
CREATE TABLE IF NOT EXISTS certificate_analytics (
    id VARCHAR(36) PRIMARY KEY DEFAULT (UUID()),
    certificate_id VARCHAR(36) REFERENCES certificates(id) ON DELETE CASCADE,
...
-- CREATE POLICY REMOVED:  ON certificate_analytics
...
CREATE OR REPLACE FUNCTION increment_linkedin_shares(cert_id TEXT)
RETURNS VOID AS $$
BEGIN
    UPDATE certificates 
    ...
    INSERT INTO certificate_analytics (certificate_id, action_type)
    SELECT id, 'linkedin_share' FROM certificates WHERE certificate_id = cert_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 9. Function to track verification
CREATE OR REPLACE FUNCTION track_certificate_verification(cert_id TEXT, ip TEXT DEFAULT NULL, ua TEXT DEFAULT NULL)
RETURNS VOID AS $$
BEGIN
    UPDATE certificates 
    ...
    INSERT INTO certificate_analytics (certificate_id, action_type, ip_address, user_agent)
    SELECT id, 'verification', ip, ua FROM certificates WHERE certificate_id = cert_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
